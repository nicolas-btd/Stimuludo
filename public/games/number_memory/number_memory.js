...
const number_memoryTranslations = {
    fr: {
        header_title: "MÉMOIRE DE NOMBRES",
        intro: {
            ranked: "CLASSÉ",
            sandbox: "LIBRE"
        },
        settings: {
            spd_1000: "1000ms / chiffre",
            spd_500: "500ms / chiffre",
            title: "Configuration",
            lives: "Vies :",
            lives_hint: "0 = Vies infinies.",
            display_time: "Temps additionnel :",
            display_time_sandbox: "Temps par chiffre (ms) :",
            display_time_hint: "S'ajoute à chaque niveau.",
        ...
        results: {
            ...
            level_reached: "Chiffres mémorisés",
            record: "Record :",
            best: "Meilleur",
            you: "Vous",
    ...
    en: {
        header_title: "NUMBER MEMORY",
        intro: {
            ranked: "RANKED",
            sandbox: "SANDBOX"
        },
        settings: {
            spd_1000: "1000ms / digit",
            spd_500: "500ms / digit",
            title: "Settings",
            lives: "Lives:",
            lives_hint: "0 = Infinite lives.",
            display_time: "Additional time:",
            display_time_sandbox: "Time per digit (ms):",
            display_time_hint: "Added at each level.",
        ...
        results: {
            ...
            level_reached: "Digits Memorized",
            record: "Best:",
            best: "Best",
            you: "You",
            avg_time: "Avg Time",
...
if (typeof translations !== 'undefined') {
    translations.fr.number_memory = number_memoryTranslations.fr;
    translations.en.number_memory = number_memoryTranslations.en;
}
...
function getGameText(key) {
    ...
    return key.split('.').reduce((obj, k) => (obj && obj[k]) ? obj[k] : null, number_memoryTranslations[lang]) 
        || key.split('.').reduce((obj, k) => (obj && obj[k]) ? obj[k] : null, number_memoryTranslations['en']);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const SoundEngine = {
        ...
        els: {
            ...
            numberWrapper: document.getElementById('number-wrapper'),
        ...
        init() {
            ...
            if (this.els.newRecordMsg && this.els.viewRanked) {
                ...
                this.els.numberWrapper.classList.remove('blurred');
                this.startGame();
            } else {
                this.setPreStartState();
            }
            const initKey = `nm_ranked_${this.state.savedRankedConfig}`;
        ...
        loadUserPreferences() {
            ...
            const savedRanked = localStorage.getItem('nm_ranked_config');
            if (savedRanked === '1_1000' || savedRanked === '1_500') {
                if(c.rankedConfig) c.rankedConfig.value = savedRanked;
                this.state.savedRankedConfig = savedRanked;
            } else {
                if(c.rankedConfig) c.rankedConfig.value = "1_1000";
                this.state.savedRankedConfig = "1_1000";
                localStorage.setItem('nm_ranked_config', '1_1000');
            }

            const savedLives = localStorage.getItem('nm_sandbox_lives');
            if (savedLives !== null) {
                if(c.lives) c.lives.value = savedLives;
                this.state.savedSandboxLives = parseInt(savedLives);
            }

            const savedTime = localStorage.getItem('nm_sandbox_time');
            if (savedTime !== null) {
        ...
        setPreStartState() {
            ...
            this.els.numberWrapper.classList.remove('hidden');
            this.els.numberWrapper.classList.add('blurred'); 
        ...
        startGame() {
            ...
            this.els.numberWrapper.classList.remove('hidden');
                ...
                const parts = this.state.savedRankedConfig.split('_');
                this.state.configLives = parseInt(parts[0]);
                this.state.timePerDigit = parseInt(parts[1]);
                this.state.baseTime = 1000;
        ...
        startLevel() {
            this.clearTimers();
            this.els.numberInput.value = "";
            this.els.numberInput.classList.remove('input-success', 'input-error');

                        this.state.currentNumber = this.generateNumber(this.state.level);
            this.els.numberDisplay.textContent = this.state.currentNumber;

                        const displayDuration = this.state.baseTime + (this.state.level * this.state.timePerDigit);
        ...
        handleAnswerSubmit() {
            ...
            const answer = this.els.numberInput.value.trim();
            if (answer === "") return; 
        ...
        loseLife(wrongAnswer) {
            ...
            if (this.state.configLives !== 0) {
                ...
                window.LivesManager.update(this.els.livesContainer, this.state.lives, this.state.configLives);
                        ...
                        this.els.numberInput.classList.add('input-error');

                        setTimeout(() => {
                this.els.feedbackTitle.textContent = getGameText('game.wrong_title');
                this.els.feedbackCorrect.textContent = this.state.currentNumber;
        ...
        levelComplete() {
            ...
            this.els.numberInput.classList.add('input-success');
        ...
        async gameOver() {
            ...
            SoundEngine.playEnd();
                ...
                this.els.rankedScore.textContent = this.state.level;
                                ...
                                this.saveScoreToDatabase(this.state.level, this.state.configLives, this.state.timePerDigit);

                                const recordKey = `nm_best_ranked_${this.state.savedRankedConfig}`;
                ...
                let isRecord = false;
                    ...
                    isRecord = true;
                    this.saveBestScoreToDatabase(best, this.state.configLives, this.state.timePerDigit);
                }
                ...
                const chartKey = `nm_ranked_${this.state.savedRankedConfig}`;
        ...
        async saveScoreToDatabase(score, lives, time) {
            ...
            try {
                fetch('/api/scores', { 
                    ...
                    credentials: 'include',
                    body: JSON.stringify({ game_id: 'number_memory', score: score, details: { lives: lives, time: time }, guest_id: this.getGuestId()})
                });
        ...
        async saveBestScoreToDatabase(score, lives, time) {
            try {
                fetch('/api/best', { 
                    ...
                    credentials: 'include',
                    body: JSON.stringify({ game_mode_key: `nm_ranked_${lives}_${time}`, score: score, details: { lives: lives, time: time }, guest_id: this.getGuestId()})
                });
        ...
        async preloadDistributionData(configKey) {
            if (!configKey) configKey = `nm_ranked_${this.state.savedRankedConfig}`;
            const parts = configKey.split('_');
            const lives = parts[2];
            const time = parts[3];
            try {
                const res = await fetch(`/api/scores/distribution/number_memory?lives=${lives}&time=${time}`, {
        ...
        bindEvents() {
            ...
            els.numberInput.addEventListener('keydown', (e) => {
                ...
                if (azertyMap[e.key]) {
                    ...
                    e.target.value = val.substring(0, start) + azertyMap[e.key] + val.substring(end);
            ...
            els.settingsToggle.addEventListener('click', (e) => {
                ...
                window.history.replaceState(null, '', '?mode=' + selectedMode);
                                ...
                                if(c.rankedConfig) {
                    localStorage.setItem('nm_ranked_config', c.rankedConfig.value);
                    this.state.savedRankedConfig = c.rankedConfig.value;
                }

                                if(c.lives) {
                    localStorage.setItem('nm_sandbox_lives', c.lives.value);
                    const parsedLives = parseInt(c.lives.value, 10);
                    this.state.savedSandboxLives = isNaN(parsedLives) ? 3 : parsedLives;
                }

                                if(c.displayTime) {
                    localStorage.setItem('nm_sandbox_time', c.displayTime.value);
                    this.state.savedSandboxTime = parseInt(c.displayTime.value) || 1000;
                }

                                const configKey = `nm_ranked_${this.state.savedRankedConfig}`;
