...
const sequence_memoryTranslations = {
    fr: {
        header_title: "MÉMOIRE SÉQUENTIEL...",
        intro: {
            ranked: "CLASSÉ",
            sandbox: "LIBRE"
        },
        settings: {
            ...
            display_time_hint: "Durée d'affichage d'une case.",
            cancel: "ANNULER",
            save: "APPLIQUER"
        },
        game: {
    ...
    en: {
        ...
        settings: {
            ...
            display_time_hint: "Duration a tile lights up.",
            cancel: "CANCEL",
            save: "APPLY"
        },
        game: {
...
if (typeof translations !== 'undefined') {
    translations.fr.sequence_memory = sequence_memoryTranslations.fr;
    translations.en.sequence_memory = sequence_memoryTranslations.en;
}
...
function getGameText(key) {
    ...
    return key.split('.').reduce((obj, k) => (obj && obj[k]) ? obj[k] : null, sequence_memoryTranslations[lang]) 
        || key.split('.').reduce((obj, k) => (obj && obj[k]) ? obj[k] : null, sequence_memoryTranslations['en']);
}
    ...
    const SoundEngine = {
        ...
        playSuccess(level) {
            ...
            const t = audioCtx.currentTime + 0.15; 
                        ...
                        const baseFreq = 300 + (Math.min(level, 20) * 10); 
        ...
        loadUserPreferences() {
            ...
            const savedRanked = localStorage.getItem('sm_ranked_config');
            if (savedRanked === '1_1000' || savedRanked === '1_100') {
                c.rankedConfig.value = savedRanked;
                this.state.savedRankedConfig = savedRanked;
            } else {
                c.rankedConfig.value = "1_1000";
                this.state.savedRankedConfig = "1_1000";
                localStorage.setItem('sm_ranked_config', '1_1000');
            }

            const savedLives = localStorage.getItem('sm_sandbox_lives');
            if (savedLives !== null) {
                c.lives.value = savedLives;
                this.state.savedSandboxLives = parseInt(savedLives);
            }

            const savedTime = localStorage.getItem('sm_sandbox_time');
            if (savedTime !== null) {
        ...
        renderGrid() {
            ...
            this.els.gridContainer.style.gridTemplateColumns = `repeat(3, 1fr)`;
        ...
        startLevel(isNew = true) {
            ...
            this.state.actionTimeout = setTimeout(() => this.playSequenceStep(0), 1000);
        },
        ...
        playSequenceStep(index) {
            ...
            const tileIdx = this.state.sequence[index];
            const tile = this.els.gridContainer.children[tileIdx];

            const lightDuration = this.state.displayTime * 0.8;
            const gapDuration = this.state.displayTime * 0.2;

            SoundEngine.playTile(tileIdx);
        ...
        handleTileClick(e) {
            ...
            const expectedIndex = this.state.sequence[this.state.playerStep];

            if (index === expectedIndex) {
                SoundEngine.playTile(index);
                tile.classList.add('tile-active');
                setTimeout(() => {
                    if (this.state.isActive) tile.classList.remove('tile-active');
                }, 150);
        ...
        loseLife() {
            if (this.state.configLives !== 0) {
                ...
                this.state.isClickable = false;
                this.state.actionTimeout = setTimeout(() => this.gameOver(), 1000);
            } else {
                if (this.state.pauseRequested) {
                    ...
                    this.state.isClickable = false;
                    this.state.actionTimeout = setTimeout(() => {
                        this.els.gridContainer.classList.remove('flash-failure-grid');
                        Array.from(this.els.gridContainer.children).forEach(t => {
                            t.classList.remove('tile-error', 'tile-active');
                        });
        ...
        levelComplete() {
            this.state.isClickable = false;
                        ...
                        SoundEngine.playSuccess(this.state.level);
        ...
        async gameOver() {
            ...
            this.state.currentState = 'FINISHED';
                ...
                this.els.rankedScore.textContent = this.state.level;
                                ...
                                const recordKey = `sm_best_ranked_${this.state.savedRankedConfig}`;
                ...
                const parts = this.state.savedRankedConfig.split('_');
                const configKey = `sm_ranked_${parts[0]}_${parts[1]}`;
            ...
            if (this.els.ingameRestartBtn) this.els.ingameRestartBtn.style.visibility = 'hidden';
                        ...
                        const parts = this.state.savedRankedConfig.split('_');
            this.preloadDistributionData(`sm_ranked_${parts[0]}_${parts[1]}`);
        },
        getGuestId() {
        ...
        async preloadDistributionData(configKey) {
            if (!configKey) {
                const parts = this.state.savedRankedConfig.split('_');
                configKey = `sm_ranked_${parts[0]}_${parts[1]}`;
            }
            const parts = configKey.replace('sm_ranked_', '').split('_');
            const lives = parts[0];
            const time = parts[1];

            try {
                const res = await fetch(`/api/scores/distribution/sequence_memory?lives=${lives}&time=${time}`, {
        ...
        togglePause() {
            if (this.state.gameMode === 'ranked') return; 
                ...
                this.state.isClickable = false;
                Array.from(this.els.gridContainer.children).forEach(t => t.classList.remove('tile-active'));
            } else {
        ...
        restoreSettingsUI() {
            const c = this.els.config;
            c.rankedConfig.value = this.state.savedRankedConfig;
        ...
        bindEvents() {
            ...
            els.settingsToggle.addEventListener('click', (e) => {
                ...
                localStorage.setItem('sm_ranked_config', c.rankedConfig.value);
                localStorage.setItem('sm_sandbox_lives', c.lives.value);
                localStorage.setItem('sm_sandbox_time', c.displayTime.value);
        ...
        async saveScoreToDatabase(score, lives, time) {
            ...
            try {
                fetch('/api/scores', { 
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({ game_id: 'sequence_memory', score: score, details: { lives: lives, time: time }, guest_id: this.getGuestId() })
                });
        ...
        async saveBestScoreToDatabase(score, lives, time) {
            try {
                fetch('/api/best', { 
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({ game_mode_key: `sm_ranked_${lives}_${time}`, score: score, details: { lives: lives, time: time }, guest_id: this.getGuestId() })
                });
