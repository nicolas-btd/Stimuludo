...
const icTranslations = {
    fr: {
        header_title: "ITEMS COUNTING",
        intro: {
            ranked: "CLASSÉ",
            sandbox: "LIBRE"
        },
...
if (typeof translations !== 'undefined') {
    translations.fr.ic = icTranslations.fr;
    translations.en.ic = icTranslations.en;
}
...
function getGameText(key) {
    ...
    return key.split('.').reduce((obj, k) => (obj && obj[k]) ? obj[k] : null, icTranslations[fallbackLang])
        || key.split('.').reduce((obj, k) => (obj && obj[k]) ? obj[k] : null, icTranslations.en);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ItemsCounting = {
        state: {
            ...
            savedTimeRanked: 60,
            savedMaxPointsRanked: 15,
            savedFlashRanked: true,

            savedTimeSandbox: 60,
            savedMaxPointsSandbox: 30,
        ...
        init() {
            if (typeof VirtualNumpad !== 'undefined') {
                this.keyboard = new VirtualNumpad('#keyboard-container', {
                    onInput: (char) => {
                        ...
                        if (this.state.currentInputString.length < 3) {
                ...
                console.warn("VirtualNumpad introuvable !");
            }
            ...
            const initKey = `ic_ranked_${this.state.savedTimeRanked}_${this.state.savedMaxPointsRanked}`;
        ...
        loadUserPreferences() {
            ...
            const savedTimeR = localStorage.getItem('ic_time_ranked');
            if (savedTimeR !== null) {
                if(c.timeRanked) c.timeRanked.value = savedTimeR;
                this.state.savedTimeRanked = parseInt(savedTimeR);
            }
            const savedPointsR = localStorage.getItem('ic_points_ranked');
            if (savedPointsR !== null) {
                if(c.pointsRanked) c.pointsRanked.value = savedPointsR;
                this.state.savedMaxPointsRanked = parseInt(savedPointsR);
            }
            const savedFlashR = localStorage.getItem('ic_flash_ranked');
            if (savedFlashR !== null) {
            ...
            const savedTimeS = localStorage.getItem('ic_time_sandbox');
            if (savedTimeS !== null) {
                if(c.timeSandbox) c.timeSandbox.value = savedTimeS;
                this.state.savedTimeSandbox = parseInt(savedTimeS);
            }
            const savedPointsS = localStorage.getItem('ic_points_sandbox');
            if (savedPointsS !== null) {
                if(c.pointsSandbox) c.pointsSandbox.value = savedPointsS;
                if(c.pointsSandboxInput) c.pointsSandboxInput.value = savedPointsS;
                this.state.savedMaxPointsSandbox = parseInt(savedPointsS);
            }
            const savedMs = localStorage.getItem('ic_show_ms');
            if (savedMs !== null) {
                if(c.showMs) c.showMs.checked = (savedMs === 'true');
                this.state.savedShowMsSandbox = (savedMs === 'true');
            }
            const savedPause = localStorage.getItem('ic_allow_pause');
            if (savedPause !== null) {
                if(c.allowPause) c.allowPause.checked = (savedPause === 'true');
                this.state.savedAllowPauseSandbox = (savedPause === 'true');
            }
            const savedFlashS = localStorage.getItem('ic_flash_sandbox');
            if (savedFlashS !== null) {
        ...
        spawnItems() {
            this.els.workspace.innerHTML = '';
            const maxP = this.state.gameMode === 'ranked' ? this.state.savedMaxPointsRanked : this.state.savedMaxPointsSandbox;

            this.state.currentPoints = Math.floor(Math.random() * maxP) + 1;

            const rect = this.els.workspace.getBoundingClientRect();
            const padding = 20;
            const dotRadius = 8;
            const minDistance = dotRadius * 2 + 8; 
            const dots = [];
        ...
        checkInput(val) {
            if (val === '') return;
            const guessed = parseInt(val);

            if (guessed === this.state.currentPoints) {
                if (this.state.currentState === 'PRE_START') {
                    this.startGame();
                    return;
                }

                const timeTaken = (performance.now() - this.state.startTime) / 1000;
        ...
        async endGame() {
            ...
            if (this.state.gameMode === 'ranked') {
                ...
                this.saveScoreToDatabase(finalScore, this.state.savedTimeRanked, this.state.savedMaxPointsRanked);

                const recordKey = `ic_best_ranked_${this.state.savedTimeRanked}_${this.state.savedMaxPointsRanked}`;
                ...
                if (best === null || finalScore > best) {
                    ...
                    isRecord = true;
                    this.saveBestScoreToDatabase(best, this.state.savedTimeRanked, this.state.savedMaxPointsRanked);
                }
                ...
                const chartKey = `ic_ranked_${this.state.savedTimeRanked}_${this.state.savedMaxPointsRanked}`;
                ...
                if (timePlayed > 0 && finalScore > 0) {
                    opm = Math.round((finalScore / timePlayed) * 60);
                    avgTime = (timePlayed / finalScore).toFixed(2);
                }
        ...
        async saveScoreToDatabase(score, time, points) {
            ...
            try {
                fetch('/api/scores', {
                    ...
                    credentials: 'include',
                    body: JSON.stringify({ game_id: 'items_counting', score: score, details: { time: time, points: points }, guest_id: this.getGuestId() })
                });
            } catch (error) { console.error("Échec sauvegarde DB:", error); }
        },

        async saveBestScoreToDatabase(score, time, points) {
            try {
                fetch('/api/best', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({
                        game_mode_key: `ic_ranked_${time}_${points}`,
                        score: score,
                        details: { time: time, points: points }
                    })
                });
        ...
        async preloadDistributionData(configKey) {
            if (!configKey) configKey = `ic_ranked_${this.state.savedTimeRanked}_${this.state.savedMaxPointsRanked}`;
            const parts = configKey.split('_');
            const time = parts[2];
            const points = parts[3];
            try {
                const res = await fetch(`/api/scores/distribution/items_counting?time=${time}&points=${points}`, {
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include'
                });
                if (res.ok) {
        ...
        restoreSettingsUI() {
            ...
            if(c.pointsRanked) c.pointsRanked.value = this.state.savedMaxPointsRanked;
            ...
            if(c.pointsSandbox) c.pointsSandbox.value = this.state.savedMaxPointsSandbox;
        ...
        bindEvents() {
            ...
            if (els.saveSettings) {
                els.saveSettings.addEventListener('click', () => {
                    ...
                    if (this.state.gameMode === 'ranked') {
                        localStorage.setItem('ic_time_ranked', c.timeRanked.value);
                        localStorage.setItem('ic_points_ranked', c.pointsRanked.value);
                        localStorage.setItem('ic_flash_ranked', c.flashRanked.checked);
                        ...
                        localStorage.setItem('ic_time_sandbox', c.timeSandbox.value);
                        localStorage.setItem('ic_points_sandbox', c.pointsSandbox.value);
                        localStorage.setItem('ic_show_ms', c.showMs.checked);
                        localStorage.setItem('ic_allow_pause', c.allowPause.checked);
                        localStorage.setItem('ic_flash_sandbox', c.flashSandbox.checked);

                            this.state.savedTimeSandbox = parseInt(c.timeSandbox.value) || 0;
                        this.state.savedMaxPointsSandbox = parseInt(c.pointsSandbox.value) || 30;
